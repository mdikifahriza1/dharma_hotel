...
$sql = "SELECT id, title, slug, content, gambar, published_at FROM news WHERE is_published = 1 ORDER BY published_at";
...
<head>
  ...
  <title>Berita - Dharma Hotel</title>
  ...
  <section class="news-section">
    ...
    <div class="news-grid">
      <?php while ($row = $result->fetch_assoc()): ?>
        <div class="news-item">
          <?php if ($row['gambar']): ?>
            <img src="img/<?php echo htmlspecialchars($row['gambar']); ?>" alt="<?php echo htmlspecialchars($row['title']); ?>">
          <?php endif; ?>
          <h3><?php echo htmlspecialchars($row['title']); ?></h3>
          <p><?php echo htmlspecialchars(substr(strip_tags($row['content']), 0, 100)); ?>...</p>
          <a href="detailberita.php?slug=<?php echo urlencode($row['slug']); ?>">Baca Selengkapnya</a>
        </div>
      <?php endwhile; ?>
    </div>
  </section>
